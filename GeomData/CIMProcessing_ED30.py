...
def createSurfacePoints(current_study_name,current_study_frame) :
	...
	DS_frame,ED_frame,ES_frame,IVR_frame=tuple(current_study_frame)
	DS_frame = int(DS_frame)
	ED_frame = int(ED_frame)
	ES_frame = int(ES_frame)	
	IVR_frame = int(IVR_frame)	
	...
	os.system('rm *.*')
	dstActive = os.getcwd()
	filename = dst+'/'+ current_study_name + '_' + str(ED_frame) + '.ipnode'
	copy(filename, dstActive)
	filename = dst + '/' + current_study_name + '_' + str(ED_frame) + '.ipelem'
	copy(filename, dstActive)
	for i in range(1, DS_frame+1):
	...
	writeCMISSComFiles(filename,current_study_name, str(i))
	...
	print DS_frame
	print numIpnode
	for i in range(DS_frame, numIpnode):
		filename = dst + '/' + current_study_name + '_' + str(i) + '.ipnode'
	...
	writeCMISSComFile(filename, current_study_name, str(ED_frame))
	os.system('cm CreateSurfacePoints.com')
...
file = open('../NYStFran_FrameNumber.txt', 'r')
