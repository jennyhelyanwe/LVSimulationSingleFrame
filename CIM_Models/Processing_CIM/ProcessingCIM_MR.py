...
def convertCIMModel(current_study_name):
    ...
    if os.path.exists('model_' + current_study_name + '_ZJW'):
    	os.chdir('model_' + current_study_name + '_ZJW')
    	os.system('cp *.model ../model')
    elif os.path.exists('model_'+current_study_name):
    	os.chdir('model_' + current_study_name)
    	os.system('cp *.model ../model')
    else:
...
for i in range(no_studies):
    ...
    if current_study_name.find('STF_06')!= -1:
